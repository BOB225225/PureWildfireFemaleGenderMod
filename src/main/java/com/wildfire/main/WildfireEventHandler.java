...
public class WildfireEventHandler {
	...
	public static void registerClientEvents() {
		...
		PlayerEntityAttackCallback.EVENT.register((target) -> {
			if(target.world == null) return ActionResult.PASS;
			if(!target.world.isClient) return ActionResult.PASS;
			if(target instanceof AbstractClientPlayerEntity plr) {
				...
				if (aPlr == null) {
					...
					WildfireGender.loadGenderInfoAsync(uuid, uuid.equals(WildfireGender.MC.player.getUuid()));

					WildfireGender.MC.player.sendMessage(Text.literal("Wildfire Female Gender Mod ~ Player Enabled").formatted(Formatting.WHITE).formatted(Formatting.BOLD), true);

					//TODO: Return ActionReuslt.FAIL if hand is empty (too lazy to do it rn)
				}
			}
		...
		ClientEntityEvents.ENTITY_LOAD.register((entity, world) -> {
			...
			if(entity instanceof  AbstractClientPlayerEntity plr) {
				if(entity.getUuid() == WildfireGender.MC.player.getUuid()) {
				UUID uuid = plr.getUuid();
				GenderPlayer aPlr = WildfireGender.getPlayerById(plr.getUuid());
				if (aPlr == null) {
					aPlr = new GenderPlayer(uuid);
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			...
			if(!isVanillaServer) {
				...
				if (timer >= 5) {
					try {
						GenderPlayer aPlr = WildfireGender.getPlayerById(WildfireGender.MC.player.getUuid());
			...
			ClientPlayNetworking.registerGlobalReceiver(new Identifier(WildfireGender.MODID, "hurt"),
					(client2, handler, buf, responseSender) -> {
						...
						if(hurtSounds) {
							PlayerEntity ent = WildfireGender.MC.world.getPlayerByUuid(uuid);
							if (ent != null) {
								client.getSoundManager().play(new EntityTrackingSoundInstance(hurtSound, SoundCategory.PLAYERS, 1f, 1f, ent, timer));
							}
						}
					});
