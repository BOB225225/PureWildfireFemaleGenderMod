...
public class GenderLayer extends FeatureRenderer<AbstractClientPlayerEntity, PlayerEntityModel<AbstractClientPlayerEntity>> {
	...
	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int packedLightIn, @NotNull AbstractClientPlayerEntity ent, float limbAngle,
		...
		if (ent.isInvisibleTo(WildfireGender.MC.player)) {
		...
		try {
			...
			PlayerEntityRenderer rend = (PlayerEntityRenderer) WildfireGender.MC.getEntityRenderDispatcher().getRenderer(ent);
